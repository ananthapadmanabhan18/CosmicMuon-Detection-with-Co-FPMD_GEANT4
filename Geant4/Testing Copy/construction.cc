...
G4VPhysicalVolume *detectorconstruction::Construct(){
    ...
    //Carbon Dioxide
    G4Material* CO2 = new G4Material("CO",1.25*g/L,2);
    CO2->AddElement(nistManager->FindOrBuildElement("C"), 1);
    CO2->AddElement(nistManager->FindOrBuildElement("O"), 2);
    //Gas Mixture
    ...
    G4Box* solidDetector = new G4Box("Detector1", 100*cm,100*cm,2.5*mm);
    gaslogic = new G4LogicalVolume(solidDetector, gasMixture, "Detector1");
    new G4PVPlacement(nullptr, G4ThreeVector(0, 0*mm, 5*cm), gaslogic, "Detector1", logicworld, false, 0,checkoverlap);
